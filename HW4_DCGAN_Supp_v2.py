...
class Generator(nn.Module):
    def __init__(self, noise_dim=50, cond_dim=10, filt=64):
        ...
        # Starting dimensions are (8x8x256)
...
class Discriminator(nn.Module):
    def __init__(self, cond_dim=10, filt=64):
        ...
        # Combine image features and condition to make a single prediction of real/fake
        # Sigmoid outputs probability as [0,1]
...
def train(Gen, Disc, training_data, device, epochs=1, noise_dim=50, cond_dim=10):
    ...
    D_opt = optim.Adam(Disc.parameters(), lr=0.0002, betas=(0.5, 0.999))
    loss_fcn = nn.BCELoss()
    # D_schedule_lr = ExponentialLR(D_opt, gamma=0.95)
    ...
    for epoch in range(epochs):
        ...
        for i, (real_images, labels) in enumerate(tqdm(training_data, desc=f"Epoch {epoch + 1}/{epochs}")):
            ...
            if i % 1 == 0:
                ...
                D_loss_real = loss_fcn(output_real, real_labels)
                ...
                # All labels should be real for generated images
                fake_labels = torch.full((batch_size, 1), fake, device=device)
                
                output_fake = Disc(fake_images.detach(), cond)
                D_loss_fake = loss_fcn(output_fake, fake_labels)
            ...
            if i % 1 == 0:
                ...
                fake_images = Gen(noise, cond)
                output = Disc(fake_images, cond)
    
                # Generator loss (discriminator should classify fake as real)
                real_labels = torch.full((batch_size, 1), 1.0, device=device)
                G_loss = loss_fcn(output, real_labels)
                # if train_generator:
                G_loss.backward()
                G_opt.step()
