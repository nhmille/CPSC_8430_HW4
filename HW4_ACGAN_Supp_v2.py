...
class Discriminator(nn.Module):
    ...
    def forward(self, image, cond_input):
        ...
        cond_embedding = cond_embedding.repeat(1, 1, 4, 4)
        
        image_features = self.convDown(image)
        combined = torch.cat((image_features, cond_embedding), dim=1)
        
        sus_output = self.sus_head(combined)
        class_output = self.class_head(combined)
        
        return sus_output, class_output
...
def train(Gen, Disc, training_data, device, epochs=1, noise_dim=50, cond_dim=10):
    ...
    D_opt = optim.Adam(Disc.parameters(), lr=0.0003, betas=(0.5, 0.999))
    D_schedule_lr = ExponentialLR(D_opt, gamma=0.99)
    
    sus_loss_fcn = nn.BCELoss()
    class_loss_fcn = nn.CrossEntropyLoss() 
    ...
    for epoch in range(epochs):
        ...
        for i, (real_images, labels) in enumerate(tqdm(training_data, desc=f"Epoch {epoch + 1}/{epochs}")):
            ...
            if i % 1 == 0:
                ...
                real_class_labels = labels
                real_sus_out, real_class_out = Disc(real_images, cond)
                
                
                real_sus_loss = sus_loss_fcn(real_sus_out, real_labels)
                real_class_loss = class_loss_fcn(real_class_out, real_class_labels)
                
                # Then, train on fake images
                ...
                fake_class_labels = labels
                fake_sus_out, fake_class_out = Disc(fake_images.detach(), cond)

                fake_sus_loss = sus_loss_fcn(fake_sus_out, fake_labels)
                fake_class_loss = class_loss_fcn(fake_class_out, fake_class_labels)

                D_loss = real_sus_loss + real_class_loss + fake_sus_loss + fake_class_loss
                D_loss.backward()
                D_opt.step()
            ...
            if i % 1 == 0:
                ...
                noise = torch.randn(batch_size, noise_dim, device=device)
                fake_images = Gen(noise, cond)
                fake_sus_out, fake_class_out = Disc(fake_images, cond)
                
                fake_labels = torch.ones((batch_size, 1), device=device)
                fake_class_labels = labels
                
                G_sus_loss = sus_loss_fcn(fake_sus_out, fake_labels)
                G_class_loss = class_loss_fcn(fake_class_out, fake_class_labels)  
    
                # Combine losses for generator
                G_loss = G_sus_loss + G_class_loss
                G_loss.backward()
                G_opt.step()
    ...
    torch.save(Disc.state_dict(), "./models/dcgan_discriminator.pth")
